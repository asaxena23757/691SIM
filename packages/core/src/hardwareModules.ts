import { HardwareModel, Port, PortType, PortDirection, Requirement, Vector } from './types.js'; // Adjust this path based on your folder structure
...
export class MotorControllerModule extends GenericHardwareModule {
  constructor(id: string, position: Vector = { x: 0, y: 0 }) {
    ...
    const requirements: Requirement[] = [{ portType: PortType.POWER, minConnections: 1 }];
...
export class CANSensorModule extends GenericHardwareModule {
  constructor(id: string, position: Vector = { x: 0, y: 0 }) {
    ...
    const requirements: Requirement[] = [{ portType: PortType.CAN, minConnections: 1 }];
